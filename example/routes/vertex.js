...
router.get(
  ...
  asycErrors(async (req, res) => {
    ...
    res.render('vertex', {
      app,
      vertexName: req.params.vertex,
      appPath: schema[app].appPath,
      vertex: schema[app].resources[req.params.vertex],
    })
  }),
)
...
router.get(
  ...
  asycErrors(async (req, res) => {
    ...
    res.render('list', {
      ...
      app,
      vertexName: req.params.vertex,
      rawJSON: syntaxHighlight(JSON.stringify(list, undefined, 4)),
    })
  }),
)
...
router.get(
  ...
  asycErrors(async (req, res) => {
    const app = req.params.app.replace(/^\w/, (c) => c.toUpperCase())
    res.render(`resources/new_${pluralize.singular(req.params.vertex)}`, {
      app,
      vertexName: req.params.vertex,
      postPath: `/${req.params.app}/${req.params.vertex}`,
    })
  }),
)
...
router.post(
  ...
  asycErrors(async (req, res) => {
    ...
    const newThing = await pcoClient[app][req.params.vertex].create({
      data: {
        type: pluralize.singular(req.params.vertex),
        attributes: req.body,
      },
    })
...
router.get(
  ...
  asycErrors(async (req, res) => {
    const app = req.params.app.replace(/^\w/, (c) => c.toUpperCase())
    const item = await pcoClient[app][
      pluralize.singular(req.params.vertex)
    ].get(req.params.id)
    if (item.data.relationships.people) {
      let people = await Promise.all(
        item.data.relationships.people.data.map(async (person) => {
    ...
    res.render(`resources/resource`, {
      itemType: item.data.type.toLowerCase(),
      item: item.data,
      app,
      vertexName: req.params.vertex,
      rawJSON: syntaxHighlight(JSON.stringify(item, undefined, 4)),
    })
  }),
)
