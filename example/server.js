...
app.get('/login', (req, res) => {
  res.redirect(
    `https://api.planningcenteronline.com/oauth/authorize?client_id=${CLIENT_ID}&redirect_uri=${redirect}&response_type=code&scope=people registrations check_ins calendar giving groups services`,
  )
})
...
app.get('/oauth/callback', async (req, res) => {
  ...
  const response = await fetch(
    `https://api.planningcenteronline.com/oauth/token?grant_type=authorization_code&code=${code}&redirect_uri=${redirect}&client_id=${CLIENT_ID}&client_secret=${CLIENT_SECRET}`,
    {
      method: 'POST',
    },
  )
